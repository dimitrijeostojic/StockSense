    ...
    : IRequestHandler<GetAllProductsRequest, TResult<GetAllProductsResponse>>
{
    ...
    {
        var (items, totalCount) = await _productRepository.GetAllAsync(_currentUserAccessor.TenantPublicId, request.SearchTerm, request.SortBy, request.IsAscending, request.FilterOn, request.FilterQuery, request.PageNumber, request.PageSize, cancellationToken);
