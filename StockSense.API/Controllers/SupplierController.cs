...
public class SupplierController(IMediator mediator) : ControllerBase
{
    ...
    {
        var request = new UpdateSupplierRequest(publicId, requestBody.Name, requestBody.ContactName, requestBody.ContactEmail, requestBody.SupplierCode, requestBody.ContactPhone, requestBody.Address, requestBody.City, requestBody.Country, requestBody.Currency);
        var result = await _mediator.Send(request, cancellationToken);
        return result.ToActionResult();
    }
